...
zip_file_url = "https://databankfiles.worldbank.org/public/ddpext_download/WDI_csv.zip"
files_to_extract = ['WDICountry.csv','WDISeries.csv','WDIData.csv']
target_directory = "."
...
```{python}
result = cnx.connect().execute(text("""
with users_cti as (
select user,host from mysql.user where user like '23FA_%' and not user like '%_group%'
), tables_cti as (
...
select
  concat("grant select on 23FA_users_jdleonard.",table_name," to '",user,"'@'",host,"'; ")
from 
